...
"""
"""
因为deformable detr的cross-attention比较复杂，而且cross-mamba尚未研究，没有合适的理论分析该如何修改，暂未实现对decoder的改进，此文件等同于rtdetr_decoder.py
"""
...
class TransformerDecoderLayer(nn.Module):
    ...
    """
    和传入self.decoder的参数对应：
        target=init_ref_contents，维度为[16, 482, 256]
        reference_points=init_ref_points_unact经过sigmoid激活并且维度由[16, 482, 4]叠加到了[16, 482, 1, 4]
        memory=memory
    """
    def forward(self,
                target,
...
@register()
class Mamba2_Decoder(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        """
        下面是IoU-aware query-selection
        init_ref_content: 查询内容，用于解码器的输入，包含了num_queries=300个tok-k查询内容和182个带噪声的查询向量。init_ref_contents.shape=torch.Size([16, 482, 256])，解释：denoising_logits.shape=torch.Size([16, 182, 256])
            更进一步解释暂时不带噪声的查询内容[16,300,256]：
            300是指模型在每个样本中需要处理或预测的候选对象数量；
            256表示每个查询对象的特征信息，特征向量的长度越长，模型能够捕获的信息就越丰富。
        init_ref_points_unact: 边界框坐标（未激活），包含了num_queries=300个tok-k边界框和182个带噪声的边界框。init_ref_points_unact.shape=torch.Size([16, 482, 4])
        enc_topk_bboxes_list: 在训练模式下，存储了top-k的边界框坐标。enc_topk_bboxes_list[0].shape=torch.Size([16, 300, 4])
        enc_topk_logits_list: 在训练模式下，存储了top-k的logits。enc_topk_logits_list[0].shape=torch.Size([16, 300, 80])
        """
